...
$result = mysqli_query($db,"SELECT * FROM customer where custid='$_SESSION[custid]' ");	
$rowrec = mysqli_fetch_array($result);


$result1 = mysqli_query($db,"SELECT * FROM service where custid='$_SESSION[custid]' AND status='Completed' ");

$result2 = mysqli_query($db,"SELECT * FROM service where custid='$_SESSION[custid]' AND status='Pending' ");
	
$result3 = mysqli_query($db,"SELECT * FROM vehicle where custid='$_SESSION[custid]' AND status='Accepted' ");

$result4 = mysqli_query($db,"SELECT * FROM vehicle where custid='$_SESSION[custid]' AND status='Pending' ");

$result5 = mysqli_query($db,"SELECT * FROM booking where custid='$_SESSION[custid]' ");
